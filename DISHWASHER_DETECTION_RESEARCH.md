...
## 8. Embeddable Local Vision Model — Training Plan

The problem statement calls for a small, embeddable model that runs locally
(free and fast, no external API).  The plan below targets a **≤ 10 MB ONNX
file** that can be bundled inside the project and served by the FastAPI backend
without any GPU requirement.

---

### 8.1 Chosen Architecture — MobileNetV3-Small Fine-tuned Binary Classifier

| Property | Value |
|----------|-------|
| Base model | MobileNetV3-Small (ImageNet pre-trained, ~1.5 M params) |
| Export format | ONNX (CPU inference via `onnxruntime`) |
| Model size | ~9 MB (ONNX FP32) |
| Inference speed | ~15–30 ms per image on a modern CPU |
| Training framework | PyTorch + `torchvision` |
| Task | Binary classification: `dishwasher` / `no_dishwasher` |

MobileNetV3-Small is chosen over YOLOv8n because:
- We only need *presence* detection (not bounding boxes) for this project.
- Classification is simpler to train, validate, and export.
- The smaller size makes it easy to commit alongside the code.

---

### 8.2 Dataset

**Primary source — Google Open Images v7**

```bash
# Install the Open Images downloader
pip install openimages
```

```python
# download_dataset.py — run once to populate data/open_images/
from openimages.download import download_dataset

download_dataset("data/open_images", ["Dishwasher"], limit=4000)
```

**Negative examples** (kitchens without a dishwasher):

Use the Open Images `Kitchen` and `Kitchen & dining room table` classes minus
any image that also carries the `Dishwasher` label.

| Split | Dishwasher | No dishwasher |
|-------|-----------|---------------|
| train | 3 000 | 3 000 |
| val | 500 | 500 |
| test | 500 | 500 |
...
val_ds   = ImageFolder("data/val",   transform=transform_val)
...
# Replace the final classifier for 2-class output
...
torch.onnx.export(
    model, dummy, "backend/models/dishwasher_classifier.onnx",
    input_names=["image"],
    output_names=["logits"],
    dynamic_axes={"image": {0: "batch"}},
    opset_version=17,
)
print("Exported dishwasher_classifier.onnx")
```

Expected training time: ~20 minutes on a single GPU (or ~4 hours on CPU).

---

### 8.4 Inference in the Backend

```python
# backend/vision_checker.py  (v0.2 addition)
import numpy as np
import onnxruntime as ort
from PIL import Image
import httpx

_MODEL_PATH = "backend/models/dishwasher_classifier.onnx"
...
def has_dishwasher_in_image(image_url: str) -> bool:
    resp = httpx.get(image_url, timeout=10)
    img  = Image.open(httpx.BytesIO(resp.content))
    arr  = _preprocess(img)
    logits = _SESSION.run(["logits"], {"image": arr})[0]   # (1, 2)
    return int(np.argmax(logits)) == 1   # class 1 = dishwasher
```

Integrating this into `main.py` requires only:
1. Calling `has_dishwasher_in_image` for each image URL from the scraper.
2. Short-circuiting on the first `True`.

---

### 8.5 Accuracy Targets and Evaluation

| Metric | Target |
|--------|--------|
| Precision (dishwasher) | ≥ 0.90 |
| Recall (dishwasher) | ≥ 0.85 |
| False-positive rate | ≤ 0.10 |
| Inference latency (CPU) | ≤ 50 ms/image |

Use a held-out test set of real Zillow kitchen photos (not Open Images) for
final evaluation to measure generalisation to in-the-wild listing photography.

---

### 8.6 Model Storage

The trained ONNX file lives at `backend/models/dishwasher_classifier.onnx`
(~9 MB) and is committed to the repository so the backend has no external
download step at startup.  Git LFS is recommended if the file exceeds 50 MB.
